...
var NRS = (function (NRS, $, undefined) {
    ...
    /* MUTAPA BANK PAGE */
    /* Mutapa Bank Page Search capitalization */
    ...
    search.on("submit", function (e, data) {
        ...
        async.waterfall([
            ...
            function(callback) {
                ...
                if (NRS.accountInfo.unconfirmedBalanceMTA == "0") {
                    $("#ms_your_nxt_balance").html("0");
                } else {
                    $("#ms_your_nxt_balance").html(NRS.formatAmount(NRS.accountInfo.unconfirmedBalanceMTA));
                }
                NRS.pageLoaded();
                callback(null);
            }
    ...
    function getAllCurrenciesRows(response) {
        ...
        for (var i = 0; i < response.currencies.length; i++) {
            ...
            var minReserve = NRS.escapeRespStr(currency.minReservePerUnitMTA);
    ...
    NRS.getTypeIcons = function (type) {
        ...
        if (NRS.isMintable(type)) {
            typeIcons += "<i title='" + $.t('mintable') + "' class='far fa-money-wave-alt'></i> ";
        }
        return typeIcons;
    };
    ...
    function processOffers(offers, type, refresh) {
        ...
        if (offers && offers.length) {
            ...
            var rateMTADecimals = NRS.getNumberOfDecimals(offers, "rateMTAPerUnit", function(offer) {
                return NRS.convertToQNTf(offer.rateMTAPerUnit, NRS.getActiveChainDecimals());
            });
            for (var i = 0; i < offers.length; i++) {
                var offer = offers[i];
                var rateMTAPerUnit = offer.rateMTAPerUnit;
                if (i == 0 && !refresh) {
                    $("#" + (type == "sell" ? "buy" : "sell") + "_currency_rate").val(NRS.formatQuantity(rateMTAPerUnit, NRS.getActiveChainDecimals()));
                ...
                rows += "<tr>" +
                    ...
                    "<td class='numeric'>" + NRS.formatQuantity(rateMTAPerUnit, NRS.getActiveChainDecimals(), false, rateMTADecimals) + "</td>" +
                    "</tr>";
            }
    ...
    NRS.loadCurrencyOffers = function (type, currencyId, refresh) {
        ...
        function(err, results) {
            ...
            offers.sort(function (a, b) {
                if (type == "sell") {
                    return a.rateMTAPerUnit - b.rateMTAPerUnit;
                } else {
                    return b.rateMTAPerUnit - a.rateMTAPerUnit;
                }
            });
    ...
    NRS.incoming.mutapa_bank = function () {
    ...
    foundersModal.on("show.bs.modal", function (e) {
        ...
        NRS.sendRequest("getCurrencyFounders", {
            ...
            var minReservePerUnitMTA = new BigInteger(NRS.floatToInt(String($invoker.data("minreserve")), decimals));
            ...
            if (response.founders && response.founders.length) {
                var amountPerUnitMTA = BigInteger.ZERO;
                for (var i = 0; i < response.founders.length; i++) {
                    amountPerUnitMTA = new BigInteger(response.founders[i].amountPerUnitMTA);
                    totalAmountReserved = totalAmountReserved.add(amountPerUnitMTA);
                }
                for (i = 0; i < response.founders.length; i++) {
                    amountPerUnitMTA = new BigInteger(response.founders[i].amountPerUnitMTA);
                    var percentage = NRS.calculatePercentage(amountPerUnitMTA, minReservePerUnitMTA);
                    rows += "<tr>" +
                    "<td>" + NRS.getAccountLink(response.founders[i], "account")+ "</td>" +
                    "<td>" + NRS.convertToNXT(amountPerUnitMTA) + "</td>" +
                    "<td>" + NRS.convertToNXT(amountPerUnitMTA.multiply(new BigInteger(NRS.convertToQNTf(resSupply, decimals)))) + "</td>" +
                    "<td>" + NRS.formatQuantity(resSupply.subtract(initialSupplyQNT).multiply(amountPerUnitMTA).divide(totalAmountReserved), decimals) + "</td>" +
                    "<td>" + percentage + "</td>" +
                    "</tr>";
                }
            } else {
            ...
            "<td>" + NRS.calculatePercentage(totalAmountReserved, minReservePerUnitMTA) + "</td>" +
            "</tr>";
        ...
        if (NRS.currenciesTradeHistoryType == "my_exchanges" && !tableName) {
            NRS.sendRequest("getExchanges+", {
                ...
                if (response.exchanges && response.exchanges.length) {
                    ...
                    var rateMTADecimals = NRS.getNumberOfDecimals(response.exchanges, "rateMTAPerUnit", function(exchange) {
                        return NRS.formatQuantity(exchange.rateMTAPerUnit, NRS.getActiveChainDecimals());
                    });
                    var totalMTADecimals = NRS.getNumberOfDecimals(response.exchanges, "totalMTA", function(exchange) {
                        return NRS.formatQuantity(NRS.multiply(exchange.unitsQNT, exchange.rateMTAPerUnit), decimals + NRS.getActiveChainDecimals());
                    });
                    for (var i = 0; i < response.exchanges.length; i++) {
                        ...
                        rows += "<tr>" +
                            ...
                            "<td class='numeric'>" + NRS.formatQuantity(exchange.rateMTAPerUnit, NRS.getActiveChainDecimals(), false, rateMTADecimals) + "</td>" +
                            "<td class='numeric'>" + NRS.formatQuantity(NRS.multiply(exchange.unitsQNT, exchange.rateMTAPerUnit), decimals + NRS.getActiveChainDecimals(), false, totalMTADecimals) + "</td>" +
                        "</tr>";
                    }
    ...
    function processExchangeRequests(exchangeRequests, refresh) {
        ...
        if (exchangeRequests && exchangeRequests.length) {
            ...
            var rateMTADecimals = NRS.getNumberOfDecimals(exchangeRequests, "rateMTAPerUnit", function(exchangeRequest) {
                return NRS.formatQuantity(exchangeRequest.rateMTAPerUnit, NRS.getActiveChainDecimals());
            });
            var totalMTADecimals = NRS.getNumberOfDecimals(exchangeRequests, "totalMTA", function(exchangeRequest) {
                return NRS.formatQuantity(NRS.multiply(exchangeRequest.unitsQNT, exchangeRequest.rateMTAPerUnit), NRS.getActiveChainDecimals() + decimals);
            });
            for (var i = 0; i < exchangeRequests.length; i++) {
                ...
                rows += "<tr class=confirmed>" +
                    ...
                    "<td class='numeric'>" + NRS.formatQuantity(exchangeRequest.rateMTAPerUnit, NRS.getActiveChainDecimals(), false, rateMTADecimals) + "</td>" +
                    "<td class='numeric'>" + NRS.formatQuantity(NRS.multiply(exchangeRequest.unitsQNT, exchangeRequest.rateMTAPerUnit), NRS.getActiveChainDecimals() + decimals, false, totalMTADecimals) + "</td>" +
                    "</tr>";
            }
    ...
    $("#currency_order_modal").on("show.bs.modal", function (e) {
        ...
        var rateMTAPerUnit = $invoker.data("rateMTAPerUnit");
        var effectiveRate = $invoker.data("effectiveRate");
        var totalMTA = $invoker.data("totalMTA");
        var totalCoin = NRS.formatAmount(totalMTA, false, true);
        ...
        if (rateMTAPerUnit == "0" || unitsQNT == "0") {
        ...
        $("#currency_order_rate").val(rateMTAPerUnit);
    });
    ...
    unitFields.on("change", function() {
        ...
        NRS.sendRequest("getAvailableTo" + NRS.initialCaps(orderType), {
            ...
            var rate = NRS.convertToQNTf(response.rateMTAPerUnit, NRS.getActiveChainDecimals());
            rateField.val(rate);
            var amount = NRS.convertToNXT(response.amountMTA);
            totalField.val(amount);
            NRS.sendRequest("getBalance", {
                "account": NRS.accountRS
            }, function (balance) {
                if (parseInt(response.amountMTA) > parseInt(balance.unconfirmedBalanceMTA)) {
            ...
            submitButton.data("rateMTAPerUnit", response.rateMTAPerUnit);
            submitButton.data("effectiveRate", effectiveRate);
            submitButton.data("totalMTA", response.amountMTA);
            submitButton.prop('disabled', false);
        })
   	});
    ...
    NRS.pages.exchange_history = function () {
        NRS.sendRequest("getExchanges+", {
            ...
            if (response.exchanges && response.exchanges.length) {
                ...
                var totalMTADecimals = NRS.getNumberOfDecimals(response.exchanges, "totalMTA", function(exchange) {
                    return NRS.formatQuantity(NRS.multiply(exchange.unitsQNT, exchange.rateMTAPerUnit), exchange.decimals + NRS.getActiveChainDecimals());
                });
                var rows = "";
                for (var i = 0; i < response.exchanges.length; i++) {
                    var exchange = response.exchanges[i];
                    ...
                    "<td class='numeric'>" + NRS.formatQuantity(exchange.unitsQNT, exchange.decimals, false, quantityDecimals) + "</td>" +
                    "<td class='numeric'>" + NRS.formatQuantity(exchange.rateMTAPerUnit, NRS.getActiveChainDecimals(), false, rateMTADecimals) + "</td>" +
                    "<td class='numeric'>" + NRS.formatQuantity(NRS.multiply(exchange.unitsQNT, exchange.rateMTAPerUnit),exchange.decimals + NRS.getActiveChainDecimals(),false,totalMTADecimals) + "</td>" +
                    "</tr>";
                }
                NRS.dataLoaded(rows);
            } else {
    ...
    NRS.setup.currencies = function () {
        var sidebarId = 'sidebar_mutapa_bank';
        var options = {
            "id": sidebarId,
            "titleHTML": '<i class="fa fa-money-bill"></i><span data-i18n="mutapa_bank">Mutapa Bank</span>',
            "page": 'currencies',
            "desiredPosition": 40,
    ...
    NRS.forms.publishExchangeOffer = function ($modal) {
        ...
        data.buyRateMTAPerUnit = NRS.convertToMTA(data.buyRateMTAPerUnit);
        ...
        data.sellRateMTAPerUnit = NRS.convertToMTA(data.sellRateMTAPerUnit);
        return {
            "data": data
        };
    };
    ...
    $("#reserve_currency_modal").on("show.bs.modal", function (e) {
        ...
        NRS.sendRequest("getCurrency+", {
            ...
            if (response && !response.errorDescription) {
                ...
                var minReserve = response.minReservePerUnitMTA;
                var currentReserve = response.currentReservePerUnitMTA;
                ...
                var minReservePerUnitMTA = new BigInteger(minReserve).multiply(new BigInteger("" + Math.pow(10, decimals)));
                $("#reserve_currency_minReserve_text").html(NRS.formatQuantity(NRS.convertToNXT(minReservePerUnitMTA.multiply(new BigInteger(resSupply))), decimals));
                $("#reserve_currency_currentReserve").val(currentReserve);
                var currentReservePerUnitMTA = new BigInteger(currentReserve);
                $("#reserve_currency_currentReserve_text").html(NRS.formatQuantity(NRS.convertToNXT(currentReservePerUnitMTA.multiply(new BigInteger(resSupply))), decimals));
    ...
    reserveCurrencyAmount.blur(function () {
        ...
        var amountMTA = NRS.convertToMTA(this.value);
        var unitAmountMTA = new BigInteger(amountMTA).divide(new BigInteger(resSupply));
        var roundUnitAmountMTA = NRS.convertToMTA(NRS.amountToPrecision(NRS.convertToNXT(unitAmountMTA), decimals));
        $("#reserve_currency_total").val(NRS.convertToNXT(roundUnitAmountMTA));
        reserveCurrencyAmount.val(NRS.convertToNXT(new BigInteger(roundUnitAmountMTA).multiply(new BigInteger(resSupply)).toString()));
    });

    NRS.forms.currencyReserveIncrease = function ($modal) {
        var data = NRS.getFormData($modal.find("form:first"));
        data.amountPerUnitMTA = NRS.convertToMTA(data.amountPerUnitMTA);
        return {
            "data": data
        };
    };
    ...
    $("#claim_currency_modal").on("show.bs.modal", function (e) {
        ...
        NRS.sendRequest("getCurrency", {
            ...
            var currentReservePerUnitMTA = new BigInteger(response.currentReservePerUnitMTA);
            $("#claimRate").html(NRS.formatAmount(currentReservePerUnitMTA) + " [" + NRS.getActiveChainName() + "/" + currencyCode + "]"); // TODO test
        });
