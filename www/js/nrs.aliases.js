...
var NRS = (function (NRS, $, undefined) {
    ...
    NRS.setup.aliases = function () {
        var options = {
            "id": 'sidebar_aliases',
            "titleHTML": '<i class="fa fa-user-secret"></i> <span data-i18n="aliases">Aliases</span>',
            "page": 'aliases',
            "desiredPosition": 100,
        ...
        if (data.priceMTA == "0") {
            ...
            $.growl(
                $.t("success_alias_sell", {
                    ...
                    "price": NRS.convertToNXT(data.priceMTA).escapeHTML()
                }), {
                    "type": "success"
                }
            );
        ...
        NRS.sendRequest("getAlias", {
            ...
            if (response.errorCode) {
                ...
                if (!("priceMTA" in response)) {
                    ...
                    $modal.find("input[name=amountNXT]").val(NRS.convertToNXT(response.priceMTA)).prop("readonly", true);
                }
            }
        }, { isAsync: false });
    });
    ...
    NRS.forms.buyAliasComplete = function (response, data) {
        ...
        $.growl(
            $.t("success_alias_buy", {
                "alias_name": data.aliasName,
                "price": NRS.convertToNXT(data.amountMTA).escapeHTML()
            }), {
                "type": "success"
            }
        );
                    ...
                    if ("priceMTA" in response) {
                        if (response.buyer == NRS.account) {
                            message = $.t("alias_sale_direct_offer", {
                                "nxt": NRS.formatAmount(response.priceMTA), "coin": NRS.getActiveChainName()
