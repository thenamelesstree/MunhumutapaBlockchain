...
var NRS = (function(NRS, $) {
	...
	NRS.getmStoreItemHTML = function(good) {
		var html = "";
			...
			html += "<table width='100%' style='table-layout:fixed;'>";
				html += "<tr>";
					...
					html += "<td>";
						...
						html += "<div class='price'><strong>" + NRS.formatAmount(good.priceMTA) + " " + NRS.getActiveChainName() + "</strong></div>";
	...
	NRS.getmStorePurchaseHTML = function(purchase, showBuyer) {
		...
		return "<div data-purchase='" + NRS.escapeRespStr(purchase.purchase) + "' " + (purchase.unconfirmed ? "class='tentative'" : "") + "><div style='float:right;color: #999999;background:white;padding:5px;border:1px solid #ccc;border-radius:3px'>" +
			...
			"<tr><td><strong>" + $.t("price") + "</strong>:</td><td>" + NRS.formatAmount(purchase.priceMTA) + " " + NRS.getActiveChainName() + "</td></tr>" +
	...
	NRS.getmStorePendingOrderHTML = function(purchase) {
	...
	NRS.dgs_show_results = function(response) {
		...
		if (response.goods && response.goods.length) {
			...
			for (var i = 0; i < response.goods.length; i++) {
				content += NRS.getmStoreItemHTML(response.goods[i]);
			}
		}
		...
		NRS.sendRequest("getDGSTags+", {
				...
				if (response.tags && response.tags.length) {
					...
					for (var i=0; i<response.tags.length; i++) {
						content += '<div style="padding:10px 24px 5px 24px;text-align:center;background-image:linear-gradient(to right, #FFFFFF 0%, #FFFFFF 100%);border-radius: 30px;-webkit-appearance:none;font-size:24px;';
						content += 'width:220px;display:inline-block;margin:2px;border:1px solid #A6B8B8;">';
		...
		NRS.sendRequest("getDGSPurchases+", {
			...
			if (response.purchases && response.purchases.length) {
				...
				for (var i = 0; i < response.purchases.length; i++) {
					content += NRS.getmStorePurchaseHTML(response.purchases[i]);
				}
			}
			NRS.dataLoaded(content);
		});
	...
	NRS.setup.dgs_search = function() {
		...
		var options = {
			"id": sidebarId,
			"titleHTML": '<i class="fa fa-bags-shopping"></i><span data-i18n="mstore">mStore</span>',
			"page": 'dgs_search',
			"desiredPosition": 60,
		...
		options = {
			"titleHTML": '<span data-i18n="mstore">mStore</span></a>',
			"type": 'PAGE',
			"page": 'dgs_search'
		};
	...
	NRS.pages.completed_orders_dgs = function() {
		NRS.sendRequest("getDGSPurchases+", {
			...
			if (response.purchases && response.purchases.length) {
				...
				for (var i = 0; i < response.purchases.length; i++) {
					content += NRS.getmStorePurchaseHTML(response.purchases[i], true);
				}
			}
	...
	NRS.pages.pending_orders_dgs = function() {
		NRS.sendRequest("getDGSPendingPurchases+", {
			...
			if (response.purchases && response.purchases.length) {
				...
				for (var i = 0; i < response.purchases.length; i++) {
					content += NRS.getmStorePendingOrderHTML(response.purchases[i]);
				}
			}
			NRS.dataLoaded(content);
		});
         ...
         var rows = "";
			if (response.goods && response.goods.length) {
				...
				for (var i = 0; i < response.goods.length; i++) {
					...
					rows += "<td class='price numeric'>" + NRS.formatAmount(good.priceMTA, false, false, priceDecimals) + " " + NRS.getActiveChainName() + "</td>";
    ...
    NRS.forms.dgsListingComplete = function (response, data) {
        ...
        if (NRS.currentPage == "my_dgs_listings") {
            var rowToAdd = "<tr class='tentative' data-goods='" + NRS.fullHashToId(response.fullHash) + "'>" +
                ...
                "<td class='price'>" + NRS.formatAmount(data.priceMTA) + " " + NRS.getActiveChainName() + "</td>" +
	...
	NRS.forms.dgsPriceChangeComplete = function(response, data) {
		...
		$("#my_dgs_listings_table").find("tr[data-goods=" + String(data.goods).escapeHTML() + "]").addClass("tentative").find(".price").html(NRS.formatAmount(data.priceMTA) + " " + NRS.getActiveChainName());
	};
			...
			if (response.errorCode) {
				...
				NRS.sendRequest("getDGSGood", {
					...
					if (good.errorCode) {
						...
						output += "<tr><th><strong>" + $.t("price") + "</strong>:</th><td>" + NRS.formatAmount(response.priceMTA) + " " + NRS.getActiveChainName() + "</td></tr>";
						...
						if (type == "dgs_refund_modal" || type == "dgs_delivery_modal" || type == "dgs_feedback_modal") {
							...
							if (response.quantity != "1") {
								var orderTotal = NRS.formatAmount(new BigInteger(String(response.quantity)).multiply(new BigInteger(String(response.priceMTA))));
								output += "<tr><th><strong>" + $.t("total") + "</strong>:</th><td>" + orderTotal + " " + NRS.getActiveChainName() + "</td></tr>";
							}
							if (response.discountMTA && (type == "dgs_refund_modal" || type == "dgs_feedback_modal")) {
								output += "<tr><th><strong>" + $.t("discount") + "</strong>:</th><td>" + NRS.formatAmount(response.discountMTA) + " " + NRS.getActiveChainName() + "</td></tr>";
							}
						}
						...
						if (type == "dgs_view_refund_modal") {
							output += "<tr><th><strong>" + $.t("refund_price") + "</strong>:</th><td>" + NRS.formatAmount(response.refundMTA) + " " + NRS.getActiveChainName() + "</td></tr>";
						}
						...
						if (type == "dgs_refund_modal") {
							var orderTotalBeforeDiscount = new BigInteger(String(response.quantity)).multiply(new BigInteger(String(response.priceMTA)));
							var refund = orderTotalBeforeDiscount.subtract(new BigInteger(String(response.discountMTA)));
			...
			if (response.errorCode) {
				...
				NRS.sendRequest("getDGSGoodsPurchases+", {
					...
					if (type == "dgs_quantity_change_modal") {
						...
						$("#dgs_price_change_current_price, #dgs_price_change_price").val(NRS.convertToNXT(response.priceMTA).escapeHTML());
					} else if (type == "dgs_purchase_modal") {
						$modal.find("input[name=recipient]").val(response.sellerRS);

						$("#dgs_purchase_price").val(NRS.escapeRespStr(response.priceMTA));
						$("#dgs_total_purchase_price").html(NRS.formatAmount(response.priceMTA) + " " + NRS.getActiveChainName());

						$("#dgs_purchase_quantity").on("change", function() {
							var totalMTA = new BigInteger(response.priceMTA).multiply(new BigInteger(String($(this).val()))).toString();
							$("#dgs_total_purchase_price").html(NRS.formatAmount(totalMTA) + " " + NRS.getActiveChainName());
						});
					}
				});
			}
	...
	NRS.dgs_listings = function (table, api, full) {
    		...
    		NRS.sendRequest(api, params, function (response) {
				...
				var priceDecimals = NRS.getNumberOfDecimals(response, "priceMTA", function(val) {
					return NRS.formatAmount(val.priceMTA);
				});
    			for (var i = 0; i < response.length; i++) {
    				...
    				view.data.push({
    					...
    					"good": good,
    					"price": NRS.formatAmount(item.priceMTA, NRS.decimals, false, priceDecimals),
    					"account": account,
    					"image": image
    				})
    			}
