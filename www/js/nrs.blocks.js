...
var NRS = (function(NRS, $) {
	...
	NRS.blocksPageLoaded = function(blocks) {
		...
		for (var i = 0; i < blocks.length; i++) {
			var block = blocks[i];
			totalAmount = totalAmount.add(new BigInteger(block.totalAmountMTA));
			totalFees = totalFees.add(new BigInteger(block.totalFeeMTA));
			...
			for (var j = 0; j < block.transactions.length; j++) {
                ...
                if (NRS.isOfType(transaction, "BetaChainBlock")) {
        ...
        if (NRS.blocksPageType == "forged_blocks") {
			NRS.sendRequest("getAccountBlockCount+", {
				...
				if (response.numberOfBlocks && response.numberOfBlocks > 0) {
                    ...
                    $("#blocks_average_fee").html(NRS.formatStyledAmount(NRS.convertToMTA(avgFee))).removeClass("loading_dots");
				} else {
			...
			averageFee = NRS.convertToMTA(averageFee);
			averageAmount = NRS.convertToMTA(averageAmount);
			if (time == 0) {
