...
var NRS = (function (NRS, $) {
    ...
    NRS.multiply = function(quantityQNT, priceMTA) {
        if (typeof quantityQNT != "object") {
            quantityQNT = new BigInteger(String(quantityQNT));
        }
        if (typeof priceMTA != "object") {
            priceMTA = new BigInteger(String(priceMTA));
        }
        return quantityQNT.multiply(priceMTA).toString();
    };
    ...
    NRS.convertToMTA = function(currency) {
    ...
    NRS.formatAmountDecimals = function (amount, round, no_escaping, zeroPad, decimals) {
        ...
        if (typeof amount == "object") {
            ...
            //rounding only applies to non-mta
            if (round) {
    ...
    NRS.getTransactionsAmountDecimals = function(transactions) {
        var decimals = {};
        decimals.amount = NRS.getNumberOfDecimals(transactions, "amountMTA", function (transaction) {
            return NRS.formatAmount(transaction.amountMTA);
        });
        decimals.fee = NRS.getNumberOfDecimals(transactions, "feeMTA", function (transaction) {
            return NRS.formatAmount(transaction.feeMTA);
        });
        return decimals;
    };
