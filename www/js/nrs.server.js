...
var NRS = (function (NRS, $, undefined) {
    ...
    NRS.sendRequest = function (requestType, data, callback, options) {
        ...
        try {
            var nxtFields = [
                ...
                ["minRateNXTPerFXT", "minRateMTAPerFXT"]
            ];

            for (i = 0; i < nxtFields.length; i++) {
                var nxtField = nxtFields[i][0];
                var mtaField = nxtFields[i][1];
                if (nxtField in data) {
                    data[mtaField] = NRS.convertToMTA(data[nxtField]);
        ...
        if (hasAccountControl && hasAssetControl
            ...
            if (hasAccountControl) {
                ...
                if (maxFees > 0 && new BigInteger(data.feeMTA).compareTo(new BigInteger(phasingControl.maxFees)) > 0) {
                    callback({
                        "errorCode": 1,
    ...
    NRS.processAjaxRequest = function (requestType, data, callback, options) {
        ...
        $.ajax({
            ...
            if (secretPhrase &&
                ...
                !response.binderRateMTAPerFXT && !data.calculateFee) {
    ...
    NRS.verifyTransactionBytes = function (transactionBytes, requestType, data, attachment, isVerifyECBlock) {
        try {
            ...
            pos += 8;
            transaction.amountMTA = String(converters.byteArrayToBigInteger(byteArray, pos));
            pos += 8;
            transaction.feeMTA = String(converters.byteArrayToBigInteger(byteArray, pos));
            pos += 8;
            ...
            if (transaction.amountMTA !== data.amountMTA && !(requestType === "exchangeCoins" && transaction.amountMTA === "0")) {
                return { fail: true, param: "chain", actual: transaction.chain, expected: data.chain };
            }
    ...
    NRS.verifyTransactionTypes = function (byteArray, transaction, requestType, data, pos, attachment) {
        ...
        switch (requestType) {
            ...
            case "sellAlias":
                ...
                pos += length;
                transaction.priceMTA = String(converters.byteArrayToBigInteger(byteArray, pos));
                pos += 8;
                if (transaction.alias !== data.aliasName || transaction.priceMTA !== data.priceMTA) {
                    return { fail: true, param: requestType, actual: JSON.stringify(data), expected: JSON.stringify(transaction) };
                }
                break;
            case "buyAlias":
            ...
            case "placeBidOrder":
                ...
                transaction.quantityQNT = String(converters.byteArrayToBigInteger(byteArray, pos));
                pos += 8;
                transaction.priceMTAPerShare = String(converters.byteArrayToBigInteger(byteArray, pos));
                pos += 8;
                if (transaction.asset !== data.asset || transaction.quantityQNT !== data.quantityQNT || transaction.priceMTAPerShare !== data.priceMTAPerShare) {
                    return { fail: true, param: requestType, actual: JSON.stringify(data), expected: JSON.stringify(transaction) };
                }
                break;
            case "cancelAskOrder":
            ...
            case "dividendPayment":
                ...
                pos += 4;
                transaction.amountMTAPerShare = String(converters.byteArrayToBigInteger(byteArray, pos));
                pos += 8;
                if (transaction.holding !== data.holding ||
                    ...
                    transaction.amountMTAPerShare !== data.amountMTAPerShare) {
                    return { fail: true, param: requestType, actual: JSON.stringify(data), expected: JSON.stringify(transaction) };
                }
                break;
            ...
            case "dgsListing":
                ...
                transaction.priceMTA = String(converters.byteArrayToBigInteger(byteArray, pos));
                pos += 8;
                if (transaction.name !== data.name || transaction.description !== data.description || transaction.tags !== data.tags || transaction.quantity !== data.quantity || transaction.priceMTA !== data.priceMTA) {
                    return { fail: true, param: requestType, actual: JSON.stringify(data), expected: JSON.stringify(transaction) };
                }
                break;
            case "dgsDelisting":
            ...
            case "dgsPriceChange":
                ...
                transaction.priceMTA = String(converters.byteArrayToBigInteger(byteArray, pos));
                pos += 8;
                if (transaction.goods !== data.goods || transaction.priceMTA !== data.priceMTA) {
                    return { fail: true, param: requestType, actual: JSON.stringify(data), expected: JSON.stringify(transaction) };
                }
                break;
            ...
            case "dgsPurchase":
                ...
                pos += 4;
                if (transaction.goods !== data.goods || transaction.quantity !== data.quantity || transaction.priceMTA !== data.priceMTA || transaction.deliveryDeadlineTimestamp !== data.deliveryDeadlineTimestamp) {
                    return { fail: true, param: requestType, actual: JSON.stringify(data), expected: JSON.stringify(transaction) };
                }
                break;
            case "dgsDelivery":
                ...
                pos += 32;
                transaction.discountMTA = String(converters.byteArrayToBigInteger(byteArray, pos));
                pos += 8;
                ...
                if (transaction.purchase !== data.purchase || transaction.goodsData !== data.goodsData || transaction.goodsNonce !== data.goodsNonce || transaction.discountMTA !== data.discountMTA) {
                    return { fail: true, param: requestType, actual: JSON.stringify(data), expected: JSON.stringify(transaction) };
                }
                break;
            case "dgsFeedback":
            ...
            case "dgsRefund":
                ...
                transaction.purchase = String(converters.byteArrayToBigInteger(byteArray, pos));
                pos += 8;
                transaction.refundMTA = String(converters.byteArrayToBigInteger(byteArray, pos));
                pos += 8;
                if (transaction.purchase !== data.purchase || transaction.refundMTA !== data.refundMTA) {
                    return { fail: true, param: requestType, actual: JSON.stringify(data), expected: JSON.stringify(transaction) };
                }
                break;
            case "leaseBalance":
            ...
            case "issueCurrency":
                ...
                pos += 4;
                transaction.minReservePerUnitMTA = String(converters.byteArrayToBigInteger(byteArray, pos));
                pos += 8;
                ...
                if (transaction.minReservePerUnitMTA !== "0" && transaction.minReservePerUnitMTA !== data.minReservePerUnitMTA) {
                    return { fail: true, param: requestType + "MinReservePerUnitMTA", actual: JSON.stringify(data), expected: JSON.stringify(transaction) };
                }
            ...
            case "currencyReserveIncrease":
                ...
                pos += 8;
                transaction.amountPerUnitMTA = String(converters.byteArrayToBigInteger(byteArray, pos));
                pos += 8;
                if (transaction.currency !== data.currency || transaction.amountPerUnitMTA !== data.amountPerUnitMTA) {
                    return { fail: true, param: requestType, actual: JSON.stringify(data), expected: JSON.stringify(transaction) };
                }
                break;
            ...
            case "publishExchangeOffer":
                ...
                transaction.currency = String(converters.byteArrayToBigInteger(byteArray, pos));
                pos += 8;
                transaction.buyRateMTAPerUnit = String(converters.byteArrayToBigInteger(byteArray, pos));
                pos += 8;
                transaction.sellRateMTAPerUnit = String(converters.byteArrayToBigInteger(byteArray, pos));
                pos += 8;
                ...
                pos += 4;
                if (transaction.currency !== data.currency || transaction.buyRateMTAPerUnit !== data.buyRateMTAPerUnit || transaction.sellRateMTAPerUnit !== data.sellRateMTAPerUnit ||
            ...
            case "currencyBuy":
                ...
                transaction.currency = String(converters.byteArrayToBigInteger(byteArray, pos));
                pos += 8;
                transaction.rateMTAPerUnit = String(converters.byteArrayToBigInteger(byteArray, pos));
                pos += 8;
                transaction.unitsQNT = String(converters.byteArrayToBigInteger(byteArray, pos));
                pos += 8;
                if (transaction.currency !== data.currency || transaction.rateMTAPerUnit !== data.rateMTAPerUnit || transaction.unitsQNT !== data.unitsQNT) {
                    return { fail: true, param: requestType, actual: JSON.stringify(data), expected: JSON.stringify(transaction) };
                }
                break;
            case "currencySell":
            ...
            case "exchangeCoins":
                ...
                pos += 8;
                var priceMTAPerCoin = String(converters.byteArrayToBigInteger(byteArray, pos));
                if (priceMTAPerCoin !== data.priceMTAPerCoin) {
                    return { fail: true, param: requestType, actual: priceMTAPerCoin, expected: data.priceMTAPerCoin };
                }
                pos += 8;
                break;
            case "cancelCoinExchange":
                ...
                pos += 32;
                break;
            case "bindTransactions":
                if (NRS.notOfType(transaction, "BetaChainBlock")) {
                    return notOfTypeError;
                }
                ...
                if (chain != data.betaChain) {
                    return { fail: true, param: requestType, actual: chain, expected: data.betaChain };
                }
                pos += 4;
                serverHash = converters.byteArrayToHexString(byteArray.slice(pos, pos + 32));
                sha256 = CryptoJS.algo.SHA256.create();
                // We assume here that only one transaction was binded
