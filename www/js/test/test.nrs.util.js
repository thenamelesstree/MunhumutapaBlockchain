...
NRS.loadServerConstants(function() {
    ...
    QUnit.test("getAccountLink", function (assert) {
        ...
        assert.equal(NRS.getAccountLink({entity: 5873880488492319831}, "entity"), "<a href='#' data-user='MUNHU-XKA2-7VJU-VZSY-7R335' class='show_account_modal_action user-info'>/</a>", "numeric");
    ...
    QUnit.test("payment.request", function (assert) {
        var request = NRS.generatePaymentRequest({type: 0, id: 2, amountMTA: 123456789, message: "Payment #1"}, "hope peace happen touch easy pretend worthless talk them indeed wheel state", false);
        assert.equal(NRS.parsePaymentRequest(request).message, "Payment #1", "plain.payment.request");
        request = NRS.generatePaymentRequest({type: 0, id: 2, amountMTA: 123456789, message: "Payment #1"}, "hope peace happen touch easy pretend worthless talk them indeed wheel state", true, "584486d2ba4dbd7eaeadd071f9f8c3593cee620e1e374033551147d68899b529");
