...
var NRS = (function(NRS, $) {
            ...
            function(shufflers, callback) {
                ...
                var view = NRS.simpleview.get('active_shufflings', {
                    ...
                    shufflingWarning: $.t("shuffling_node_running_warning", {
                        deposit: NRS.formatQuantity(NRS.getActiveChain().SHUFFLING_DEPOSIT_MTA, NRS.getActiveChainDecimals()),
    ...
    NRS.forms.startShuffler = function ($modal) {
        ...
        if (data.feeRateNXTPerFXT.trim() === "") {
            return {
                "error": $.t("shuffler_binding_advice")
            };
        }
        data.feeRateMTAPerFXT = NRS.convertToMTA(data.feeRateNXTPerFXT);
    ...
    NRS.forms.startStandbyShuffler = function ($modal) {
        ...
        if (data.feeRateNXTPerFXT.trim() === "") {
            return {
                "error": $.t("standbyshuffler_binding_advice")
            };
        }
        data.feeRateMTAPerFXT = NRS.convertToMTA(data.feeRateNXTPerFXT);
        delete data.feeRateNXTPerFXT;
