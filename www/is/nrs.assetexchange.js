...
var NRS = (function (NRS, $) {
    ...
    NRS.loadAsset = function (asset, refresh) {
        ...
        if (NRS.accountInfo.unconfirmedBalanceMTA == "0") {
            ...
            $("#your_nxt_balance").html(NRS.formatAmount(NRS.accountInfo.unconfirmedBalanceMTA));
    ...
    function processOrders(orders, type, refresh) {
        if (orders.length) {
            ...
            var priceDecimals = NRS.getNumberOfDecimals(orders, "priceMTAPerShare", function(val) {
                return NRS.formatQuantity(val.priceMTAPerShare, NRS.getActiveChainDecimals());
            });
            var amountDecimals = NRS.getNumberOfDecimals(orders, "totalMTA", function(val) {
                return NRS.formatQuantity(NRS.multiply(val.quantityQNT, val.priceMTAPerShare), currentAsset.decimals + NRS.getActiveChainDecimals());
            });
            for (var i = 0; i < orders.length; i++) {
                order = orders[i];
                order.priceMTAPerShare = new BigInteger(order.priceMTAPerShare);
                order.quantityQNT = new BigInteger(order.quantityQNT);
                order.totalMTA = new BigInteger(NRS.multiply(order.quantityQNT, order.priceMTAPerShare));
                sum = sum.add(order.totalMTA);
                if (i == 0 && !refresh) {
                    $("#" + (type == "ask" ? "buy" : "sell") + "_asset_price").val(NRS.convertToQNTf(order.priceMTAPerShare, NRS.getActiveChainDecimals()));
                }
                var statusIcon = NRS.getTransactionStatusIcon(order);
                var className = (order.account == NRS.account ? "your-order" : "");
                rows += "<tr class='" + className + "' data-transaction='" + NRS.escapeRespStr(order.order) + "' data-quantity='" + order.quantityQNT.toString().escapeHTML() + "' data-price='" + order.priceMTAPerShare.toString().escapeHTML() + "'>" +
                    ...
                    "<td class='numeric'>" + NRS.formatQuantity(order.priceMTAPerShare, NRS.getActiveChainDecimals(), false, priceDecimals) + "</td>" +
                    "<td class='numeric'>" + NRS.formatQuantity(order.totalMTA, currentAsset.decimals + NRS.getActiveChainDecimals(), false, amountDecimals) + "</td>" +
    ...
    NRS.loadAssetOrders = function (type, assetId, refresh) {
        ...
        function(err, results) {
            ...
            orders.sort(function (a, b) {
                if (type == "ask") {
                    return a.priceMTAPerShare - b.priceMTAPerShare;
                } else {
                    return b.priceMTAPerShare - a.priceMTAPerShare;
                }
            });
    ...
    NRS.getAssetTradeHistory = function (assetId, refresh) {
        ...
        NRS.sendRequest("getTrades+" + assetId, options, function (response) {
            ...
            if (response.trades && response.trades.length) {
                ...
                var priceDecimals = NRS.getNumberOfDecimals(trades, "priceMTAPerShare", function(val) {
                    return NRS.formatQuantity(val.priceMTAPerShare, NRS.getActiveChainDecimals());
                });
                ...
                for (var i = 0; i < trades.length; i++) {
                    var trade = trades[i];
                    trade.priceMTAPerShare = new BigInteger(trade.priceMTAPerShare);
                    trade.quantityQNT = new BigInteger(trade.quantityQNT);
                    trade.totalMTA = new BigInteger(NRS.multiply(trade.priceMTAPerShare, trade.quantityQNT));
                    rows += "<tr>" +
                        ...
                        "<td class='asset_price numeric'>" + NRS.formatQuantity(trade.priceMTAPerShare, NRS.getActiveChainDecimals(), false, priceDecimals) + "</td>" +
                    ...
                    rows += "'>" + NRS.formatQuantity(trade.totalMTA, currentAsset.decimals + NRS.getActiveChainDecimals(), false, amountDecimals) + "</td>" +
    ...
    $("#asset_exchange_bid_orders_table tbody, #asset_exchange_ask_orders_table tbody").on("click", "td", function (e) {
        ...
        try {
            var priceMTAPerShare = new BigInteger(String($tr.data("price")));
            var quantityQNT = new BigInteger(String($tr.data("quantity")));
            var totalMTAQNT = new BigInteger(NRS.multiply(quantityQNT, priceMTAPerShare));

            $("#" + type + "_asset_price").val(NRS.convertToQNTf(priceMTAPerShare, NRS.getActiveChainDecimals()));
            $("#" + type + "_asset_quantity").val(NRS.convertToQNTf(quantityQNT, currentAsset.decimals));
            $("#" + type + "_asset_total").val(NRS.intToFloat(totalMTAQNT, NRS.getActiveChainDecimals() + currentAsset.decimals));
        } catch (err) {
            return;
        }
        ...
        if (type == "buy") {
            try {
                var balanceMTAQNT = new BigInteger(NRS.floatToInt(NRS.accountInfo.unconfirmedBalanceMTA, currentAsset.decimals));
            } catch (err) {
                NRS.logConsole("asset_exchange_bid_orders_table click " + err.message);
                return;
            }
            NRS.logConsole("asset_exchange_bid_orders_table click total " + totalMTAQNT + " balance " + balanceMTAQNT);
            if (totalMTAQNT.compareTo(balanceMTAQNT) > 0) {
    ...
    $("#sell_automatic_price, #buy_automatic_price").on("click", function () {
        try {
            ...
            var price = new Big(NRS.convertToMTA(String(assetPrice.val())));
            var balanceMTA = new Big(NRS.accountInfo.unconfirmedBalanceMTA);
            var maxQuantity = new Big(NRS.convertToQNTf(currentAsset.quantityQNT, currentAsset.decimals));
            if (balanceMTA.cmp(new Big("0")) <= 0) {
                return;
            }
            ...
            if (type == "sell") {
                ...
                quantity = new Big(NRS.amountToPrecision(balanceMTA.div(price).toString(), currentAsset.decimals));
            }
    ...
    $("#sell_asset_quantity, #sell_asset_price, #buy_asset_quantity, #buy_asset_price").keyup(function () {
        ...
        try {
            ...
            var priceMTAPerShare = new BigInteger(NRS.convertToMTA(String($("#" + orderType + "_asset_price").val())));

            if (priceMTAPerShare.toString() == "0" || quantityQNT.toString() == "0") {
                $("#" + orderType + "_asset_total").val("0");
            } else {
                var total = NRS.convertToQNTf(NRS.multiply(quantityQNT, priceMTAPerShare), currentAsset.decimals + NRS.getActiveChainDecimals());
    ...
    $("#asset_order_modal").on("show.bs.modal", function (e) {
        ...
        try {
            ...
            var priceMTAPerShare = new BigInteger(NRS.convertToMTA(price));
            var totalNXT = NRS.formatQuantity(NRS.multiply(quantityQNT, priceMTAPerShare), currentAsset.decimals + NRS.getActiveChainDecimals(), false, true);
        } catch (err) {
        ...
        if (orderType == "buy") {
            description = $.t("buy_order_description", {
                ...
                "coin_amount": NRS.formatQuantity(priceMTAPerShare, NRS.getActiveChainDecimals())
            });
        ...
        $("#asset_order_price").val(priceMTAPerShare.toString());
    });
    ...
    NRS.pages.trade_history = function () {
        NRS.sendRequest("getTrades+", {
            ...
            if (response.trades && response.trades.length) {
                ...
                var amountDecimals = NRS.getNumberOfDecimals(trades, "totalMTA", function(val) {
                    return NRS.formatQuantity(NRS.multiply(val.quantityQNT, val.priceMTAPerShare), val.decimals + NRS.getActiveChainDecimals());
                });
                var rows = "";
                for (var i = 0; i < trades.length; i++) {
                    ...
                    rows += "<tr>" +
                        ...
                        "<td class='asset_price numeric'>" + NRS.formatQuantity(trade.priceMTAPerShare, NRS.getActiveChainDecimals(), false, priceDecimals) + "</td>" +
                        "<td style='" + (type == "buy" ? "color:red" : "color:green") + "' class='numeric'>" + NRS.formatQuantity(trade.totalMTA, trade.decimals + NRS.getActiveChainDecimals(), false, amountDecimals) + "</td>" +
            ...
            for (var i = 0; i < NRS.accountInfo.assetBalances.length; i++) {
                ...
                NRS.sendRequest("getAskOrders+", {
                    ...
                    if (response.askOrders && response.askOrders.length) {
                        result.ask_orders[input.asset] = new BigInteger(response.askOrders[0].priceMTAPerShare);
                    } else {
                ...
                NRS.sendRequest("getBidOrders+", {
                    ...
                    if (response.bidOrders && response.bidOrders.length) {
                        result.bid_orders[input.asset] = new BigInteger(response.bidOrders[0].priceMTAPerShare);
                    } else {
    ...
    NRS.myAssetsPageLoaded = function (result) {
        ...
        var valueDecimals = NRS.getNumberOfDecimals(result.assets, "bid", function(asset) {
            ...
            var totalMTA = new BigInteger(NRS.multiply(asset.balanceQNT, asset.bid_orders));
            return NRS.formatQuantity(totalMTA, asset.decimals + NRS.getActiveChainDecimals());
        });
        for (var i = 0; i < result.assets.length; i++) {
            ...
            if (highestBidOrder != -1) {
                var totalMTA = new BigInteger(NRS.multiply(asset.balanceQNT, highestBidOrder));
            }
            ...
            rows += "<tr data-asset='" + assetId + "'>" +
                ...
                "<td class='numeric'>" + (highestBidOrder != -1 ? NRS.formatQuantity(totalMTA, asset.decimals + NRS.getActiveChainDecimals(), false, valueDecimals) : "") + "</td>";
            ...
            if (asset.account == NRS.account && NRS.isMutapaChain()) {
    ...
    NRS.openOrdersLoaded = function (orders, type, callback) {
        ...
        for (var i = 0; i < orders.length; i++) {
            ...
            completeOrder.priceMTAPerShare = new BigInteger(completeOrder.priceMTAPerShare);
            completeOrder.quantityQNT = new BigInteger(completeOrder.quantityQNT);
            completeOrder.totalMTA = new BigInteger(NRS.multiply(completeOrder.quantityQNT, completeOrder.priceMTAPerShare));
            rows += "<tr data-order='" + NRS.escapeRespStr(completeOrder.order) + "'>" +
                ...
                "<td>" + NRS.formatQuantity(completeOrder.priceMTAPerShare, NRS.getActiveChainDecimals()) + "</td>" +
                "<td>" + NRS.formatQuantity(completeOrder.totalMTA, completeOrder.decimals + NRS.getActiveChainDecimals()) + "</td>" +
    ...
    NRS.setup.asset_exchange = function () {
        ...
        var options = {
            "id": sidebarId,
            "titleHTML": '<i class="far fa-coin"></i><span data-i18n="assets">Assets</span>',
            "page": 'asset_exchange',
            "desiredPosition": 30,
