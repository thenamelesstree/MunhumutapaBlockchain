...
var NRS = (function (NRS, $, undefined) {
    ...
    function highlightTotal(totalMTA, balanceMTA) {
        if (totalMTA.compareTo(balanceMTA) > 0) {
            $("#buy_coin_total").css({
                "background": "#ED4348",
                "color": "white"
            });
        ...
        for (var i = 0; i < coins.length; i++) {
            ...
            if (NRS.accountInfo.balances) {
                $.each(NRS.accountInfo.balances, function(chainId, coinBalance) {
                    if (chainId == coin.id && coinBalance.unconfirmedBalanceMTA != "0") {
                        ownsCoin = true;
                        ownsCoinMTA = coinBalance.unconfirmedBalanceMTA;
                        return false;
                    }
                });
            }
            ...
            rows += "<p class='list-group-item-text'><span>" 
                ...
                + NRS.formatQuantity(ownsCoinMTA, coin.decimals) + "</span></p>";
            rows += "</a>";
        }
            ...
            for (var i = 0; i < coins.length; i++) {
                ...
                $.each(NRS.accountInfo.balances, function(chainId, coinBalance) {
                    if (chainId === coin.id && coinBalance.unconfirmedBalanceMTA != "0") {
                        ...
                        amountElement.text(": " + NRS.formatQuantity(ownsCoinMTA, coin.decimals));
                        return false;
                    }
                });
            }
    ...
    NRS.loadCoin = function (coin, refresh) {
        ...
        if (NRS.accountInfo.unconfirmedBalanceMTA == "0") {
            ...
            $("#your_coin_balance").html(NRS.formatAmount(NRS.accountInfo.unconfirmedBalanceMTA));
        }
    ...
    function processOrders(orders, coinId, refresh, action) {
        ...
        if (orders.length) {
            ...
            var rateFieldName = action == "sell" ? "askMTAPerCoin" : "bidMTAPerCoin";
            for (var i = 0; i < orders.length; i++) {
                order = orders[i];
                if (i == 0 && !refresh && action == "sell") {
                    var bestPrice = NRS.convertToQNTf(order.askMTAPerCoin, NRS.getActiveChainDecimals());
                ...
                rows += "<tr data-transaction='" + NRS.escapeRespStr(order.order) + "' data-amount='" + String(order.exchangeQNT).escapeHTML() + "' data-price='" + String(order.askMTAPerCoin).escapeHTML() + "'>" +
        ...
        async.parallel([
            ...
            function(err, results) {
                ...
                orders.sort(function (a, b) {
                    if (action == "sell") {
                        return a.askMTAPerCoin - b.askMTAPerCoin;
                    } else {
                        return b.bidMTAPerCoin - a.bidMTAPerCoin;
                    }
                });
    ...
    $("#coin_exchange_sell_orders_table").find("tbody").on("click", "td", function (e) {
        ...
        try {
            ...
            var priceMTAPerCoin = new BigInteger(priceStr);
            var amountStr = $target.data("amount") !== undefined ? String($target.data("amount")) : String($tr.data("amount"));
            NRS.logConsole("Selected rate " + priceStr + " amount " + amountStr + " balance " + NRS.accountInfo.unconfirmedBalanceMTA);
            var amountMTA = new BigInteger(amountStr);
            var totalMTA = new BigInteger(NRS.multiply(amountMTA, priceMTAPerCoin));
            $("#buy_coin_quantity").val(NRS.convertToQNTf(amountMTA, currentCoin.decimals));
            var bestPrice = NRS.convertToQNTf(priceMTAPerCoin, NRS.getActiveChainDecimals());
            $("#buy_coin_price").val(bestPrice);
            $("#buy_coin_price_inverse").val(NRS.getInverse(bestPrice));
            $("#buy_coin_total").val(NRS.amountToPrecision(NRS.intToFloat(totalMTA, currentCoin.decimals + NRS.getActiveChainDecimals()), TOTAL_FIELD_DECIMAL_PRECISION));
        } catch (err) {
        ...
        try {
            var balanceMTA = new BigInteger(NRS.floatToInt(NRS.accountInfo.unconfirmedBalanceMTA, currentCoin.decimals));
            highlightTotal(totalMTA, balanceMTA);
        } catch (err) {
            return;
        }
        NRS.logConsole("Selected totalMTA " + NRS.intToFloat(totalMTA, currentCoin.decimals + NRS.getActiveChainDecimals()) +
            " balanceMTA " + NRS.intToFloat(balanceMTA.toString(), currentCoin.decimals + NRS.getActiveChainDecimals()));
    ...
    buyCoinFields.keyup(function () {
        try {
            ...
            var priceMTAPerCoin = new BigInteger(NRS.convertToQNT(String(currentPrice), NRS.getActiveChainDecimals()));
            var totalCurrentCoin = quantityQNT.multiply(priceMTAPerCoin);
            if (priceMTAPerCoin.toString() == "0" || quantityQNT.toString() == "0") {
            ...
            var balanceMTA = new BigInteger(NRS.floatToInt(NRS.accountInfo.unconfirmedBalanceMTA, currentCoin.decimals));
            highlightTotal(totalCurrentCoin, balanceMTA);
            NRS.logConsole("quantityQNT " + quantityQNT + " priceMTAPerCoin " + priceMTAPerCoin + " total " + total);
        } catch (err) {
    ...
    $("#coin_order_modal").on("show.bs.modal", function (e) {
        ...
        try {
            ...
            var priceMTAPerCoin = NRS.convertToQNT(price, NRS.getActiveChainDecimals());
            var totalNXT = NRS.formatQuantity(NRS.multiply(quantityQNT, priceMTAPerCoin), currentCoin.decimals + NRS.getActiveChainDecimals());
            $(this).find('input[name=calculatedAmountNXT]').val(NRS.convertToQNTf(NRS.multiply(quantityQNT, priceMTAPerCoin), currentCoin.decimals + NRS.getActiveChainDecimals()));
        } catch (err) {
        ...
        description = $.t("buy_coin_order_description", {
            ...
            "price": NRS.formatAmount(priceMTAPerCoin),
        ...
        $("#coin_order_price").val(priceMTAPerCoin);
        if (currentCoin.id == "1") {
    ...
    NRS.forms.exchangeCoinsFeeCalculation = function(feeField, feeMTA) {
        if (currentCoin.id == "1") {
            feeField.val(NRS.intToFloat(feeMTA, currentCoin.decimals));
        } else {
            feeField.val(NRS.convertToNXT(feeMTA));
        }
    };

    NRS.forms.cancelCoinExchangeFeeCalculation = function(feeField, feeMTA, transaction) {
        ...
        NRS.sendRequest("getCoinExchangeOrder", { "order": order }, function(response) {
            if (response.exchange == "1") {
                feeField.val(NRS.intToFloat(feeMTA, NRS.getChainDecimals(1)));
            } else {
                feeField.val(NRS.convertToNXT(feeMTA));
            }
        });
    ...
    NRS.pages.coin_exchange_history = function () {
        ...
        NRS.sendRequest("getCoinExchangeTrades+", {
            ...
            if (response.trades && response.trades.length > 0) {
                ...
                for (var i = 0; i < trades.length; i++) {
                    var trade = trades[i];
                    var isMunhumutapaTrade = NRS.isParentChain() || trade.exchange == 1;
                    var orderChain = isMunhumutapaTrade ? 1 : NRS.getActiveChainId();
                    var matchChain = isMunhumutapaTrade ? 1 : trade.exchange;
                    trade.priceMTAPerCoin = new BigInteger(trade.priceMTAPerCoin);
                    trade.amountMTA = new BigInteger(trade.amountMTA);
                    rows += "<tr>" +
                        "<td>" + NRS.formatTimestamp(trade.timestamp) + "</td>" +
            ...
            if (response.orders && response.orders.length) {
                ...
                for (var i = 0; i < orders.length; i++) {
                    ...
                    rows += "<tr>" +
                        ...
                        "<td>" + NRS.formatQuantity(order.bidMTAPerCoin, decimals) + "</td>" +
                        "<td>" + NRS.getChainLink(order.exchange) + "</td>" +
                        "<td>" + NRS.formatQuantity(order.quantityQNT, exchangeDecimals) + "</td>" +
                        "<td>" + NRS.formatQuantity(order.askMTAPerCoin, exchangeDecimals) + "</td>";
    ...
    NRS.setup.coin_exchange = function () {
        ...
        var options = {
            "id": sidebarId,
            "titleHTML": '<i class="far fa-exchange-alt"></i><span data-i18n="coin_exchange">Coin Exchange</span>',
            "page": 'coin_exchange',
            "desiredPosition": 30,
